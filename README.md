...
## Requisitos (backend Spring)

- **JDK 21** (o superior compatible) para compilar y ejecutar la API. La versión del bytecode se fija en `pom.xml` (`<java.version>21</java.version>`). Si usas JDK 26, puedes cambiar esa propiedad a `26` **solo** si `javac` admite `--release 26`.
- El **front** (Vite/Vue) y **Strapi** siguen en **Node.js**; no usan la JVM.
...
3. **Por pasos:** SQL con `.\scripts\run-sqlserver.ps1` o `infisical run --env=dev -- docker compose up -d` → base `UCOParking` (ver [docs/PRUEBAS_CONEXION_FRONT.md](docs/PRUEBAS_CONEXION_FRONT.md)) → Spring con `.\scripts\run-spring-dev.ps1` o `infisical run --env=dev -- .\mvnw.cmd spring-boot:run` → API en `http://127.0.0.1:8080/uco-parking/api/v1`. Con **Strapi** en `:1337` y las claves `STRAPI_*` en Infisical (plantilla [docs/infisical-import-dev.env.template](docs/infisical-import-dev.env.template)), comprueba el enlace con: `infisical run --env=dev -- powershell -NoProfile -File .\scripts\verify-strapi-spring.ps1` (detalle: [docs/STRAPI.md](docs/STRAPI.md)).
